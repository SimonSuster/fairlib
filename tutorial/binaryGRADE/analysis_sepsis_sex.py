...
from data.src.Sepsis.util import get_inv_protected_label_map
...
analysis.model_selection(
    ...
    save_path=f"{Shared_options['results_dir']}/{Shared_options['project_dir']}/{Shared_options['dataset']}SexvanillaBlueBERT_df.pkl",
...
analysis.model_selection(
    # exp_id started with model_id will be treated as the same method, e.g, vanilla, and adv
    model_id=("SepsisSexVanillaClinicalBERT"),
    # the tuned hyperparameters of a methods, which will be used to group multiple runs together.
    index_column_names=["BT", "BTObj"],
    # to convenient the further analysis, we will store the resulting DataFrame to the specified path
    save_path=f"{Shared_options['results_dir']}/{Shared_options['project_dir']}/{Shared_options['dataset']}SexvanillaClinicalBERT_df.pkl",
    # Follwoing options are predefined
    results_dir=Shared_options["results_dir"],
    project_dir=Shared_options["project_dir"] + "/" + Shared_options["dataset"],
    GAP_metric_name=Shared_options["GAP_metric_name"],
...
analysis.model_selection(
    # exp_id started with model_id will be treated as the same method, e.g, vanilla, and adv
    model_id=("SepsisSexVanillaSciBERT"),
    # the tuned hyperparameters of a methods, which will be used to group multiple runs together.
    index_column_names=["BT", "BTObj"],
    # to convenient the further analysis, we will store the resulting DataFrame to the specified path
    save_path=f"{Shared_options['results_dir']}/{Shared_options['project_dir']}/{Shared_options['dataset']}SexvanillaSciBERT_df.pkl",
    # Follwoing options are predefined
    results_dir=Shared_options["results_dir"],
    project_dir=Shared_options["project_dir"] + "/" + Shared_options["dataset"],
    GAP_metric_name=Shared_options["GAP_metric_name"],
...
aurc_raw_out =True
if aurc_raw_out:
    #assert (not pareto) and (selection_criterion is None)
    with open(f"/home/simon/Apps/SysRevData/data/modelling/plots/sepsis/sepsis_aurc_raw_{task}_debias.csv", "w") as fh_out:
...
if perf_raw_out:
    ...
    with open(f"/home/simon/Apps/SysRevData/data/modelling/plots/sepsis/sepsis_perf_raw_{task}_sex_debias.csv", "w") as fh_out:

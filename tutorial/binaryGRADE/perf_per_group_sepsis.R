...
criterion = "Sepsis"
protected_group = "Sex"
file1 = paste("/home/simon/Apps/SysRevData/data/modelling/plots/sepsis/sepsis_perf_raw_", criterion, "_", tolower(protected_group), "_debias.csv", sep="")
...
#all$topic <- factor(all$topic, leveallls = all$topic[order(all$precision[all$model==paste(protected_group_substr, "vanilla", sep="")])])
...
f_out=paste("/home/simon/Apps/SysRevData/data/dataset/plots/risk_coverages_curves/sepsis/debias/recall_compare", gsub(" ", "_", criterion), tolower(protected_group), ".png", sep="")
...
all$topic <- factor(all$topic, levels = all$topic[order(all$f_macro[all$model==paste(protected_group_substr, "vanilla", sep="")])])
my = ggplot(data = all_subset, aes(x = topic, y = f_macro, group=model)) + geom_point(aes(colour = str_wrap(model, 15)), size = 0.5) +
#my = ggplot(data = all_subset, aes(x = topic, y = precision, group=model)) + geom_point(aes(colour = str_wrap(model, 15)), size = 0.5) +
geom_line(aes(colour = str_wrap(model, 15)), size = 0.5) + theme_bw() +
      #scale_y_continuous(breaks = 0:10 / 10, limits=c(0,0.45)) + xlab("protected attribute") +
    theme(plot.title = element_text(size = 14, hjust=0.5),

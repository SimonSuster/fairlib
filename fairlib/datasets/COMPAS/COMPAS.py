...
class COMPAS:
    ...
    def download_files(self):
        ...
        df[target_variable] = df.apply(lambda x: target_value if x[target_variable] == 1.0 else 'No', axis=1).astype(
            'category')

        # Process protected-column values
        race_dict = {'African-American': 'Black', 'Caucasian': 'White'}
        df['race'] = df.apply(lambda x: race_dict[x['race']] if x['race'] in race_dict.keys() else 'Other',

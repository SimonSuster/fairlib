    ...
    def train_self(self, model):
        ...
        for epoch in range(self.args.opt.adv_epochs):
            ...
            if not is_best:
                ...
                discriminator_state_dct = {j: self.subdiscriminators[j].state_dict() for j in
                                           range(self.args.adv_num_subDiscriminator)}
        ...
        best_checkpoint = torch.load(Path(self.args.model_dir) / "BEST_Discriminator.pth.tar",
        ...
        logging.info(
            ('Validation Loss: {:2.2f} \tAcc: {:2.2f} \tMacroF1: {:2.2f} \tMicroF1: {:2.2f} \t').format(
                valid_results_dic["loss"], 100. * valid_results_dic["accuracy"],
                                           100. * valid_results_dic["macro_fscore"],
                                           100. * valid_results_dic["micro_fscore"]
            ))
        logging.info(
            ('Test Loss: {:2.2f} \tAcc: {:2.2f} \tMacroF1: {:2.2f} \tMicroF1: {:2.2f} \t').format(
                test_results_dic["loss"], 100. * test_results_dic["accuracy"],
                                          100. * test_results_dic["macro_fscore"],
                                          100. * test_results_dic["micro_fscore"]
            ))
